...
public class WindDirection {
    ...
    public int setIconWind(float cod_id) {
        int answerMethodIcon = 0;
        if (cod_id >= 0 && cod_id <= 45) {
            answerMethodIcon = R.drawable.ic_arrow_top_right;
        } else if (cod_id >= 45 && cod_id <= 90) {
            answerMethodIcon = R.drawable.ic_arrow_right;
        } else if (cod_id >= 90&& cod_id <= 135) {
            answerMethodIcon = R.drawable.ic_arrow_bottom_right;
        } else if (cod_id >= 135 && cod_id <= 180) {
            answerMethodIcon = R.drawable.ic_arrow_down;
        } else if (cod_id >= 180 && cod_id <= 225) {
            answerMethodIcon = R.drawable.ic_arrow_bottom_left;
        } else if (cod_id >= 225 && cod_id <= 270) {
            answerMethodIcon = R.drawable.ic_arrow_left;
        } else if (cod_id >= 270 && cod_id <= 315) {
            answerMethodIcon = R.drawable.ic_arrow_top_left;
        } else if (cod_id >= 315&& cod_id <= 360) {
            answerMethodIcon = R.drawable.ic_arrow_up;
        }
